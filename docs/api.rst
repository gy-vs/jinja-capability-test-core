...
Tests are Python functions that take the value to the left of the test
as the first argument, and return ``True`` or ``False``. Arguments
passed to the test are passed after the value.
...
custom test, write a function that takes at least a ``value`` argument,
