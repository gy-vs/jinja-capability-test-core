...
class _FilterTestCommon(Expr):
    ...
    def as_const(self, eval_ctx=None):
        ...
        # The result of a function that takes the context or the
        # environment depends on state that can change after the
        # template was compiled, such as the environment's registry of
        # filters and tests, so it can't be folded to a constant.
        if (
            func is None
            or getattr(func, f"context{mark_name}", False) is True
            or getattr(func, f"environment{mark_name}", False) is True
        ):
            raise Impossible()

        # We cannot constant handle async functions, so we need to make
...
class Filter(_FilterTestCommon):
    ...
    filter, the other fields are the same as for :class:`Call`.
...
class Test(_FilterTestCommon):
    ...
    the other fields are the same as for :class:`Call`.
